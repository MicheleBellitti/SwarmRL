...
class RLTrainer:
    ...
    def analyze_results(self, results, idx):
        ...
        smoothed_pheromone_trail_usage = moving_average(results['pheromone_trail_usage'], window_size)
        smoothed_avg_reward_per_step = moving_average(np.array(results['total_reward']) / np.array(results['steps']), window_size)
        ...
        plt.figure()
        plt.plot(smoothed_pheromone_trail_usage, label='Pheromone Trail Usage', color='orange')
        plt.xlabel('Episode')
        plt.ylabel('Usage Ratio')
        plt.title('Pheromone Trail Usage per Episode')
        plt.legend()
        plt.savefig(f'plots/Pheromone.png_{idx}')

        # Average Reward Per Step with smoothing
        plt.figure()
        plt.plot(smoothed_avg_reward_per_step, label='Average Reward Per Step', color='green')
        plt.xlabel('Episode')
        plt.ylabel('Average Reward')
        plt.title('Average Reward Per Step per Episode')
        plt.legend()
        plt.savefig(f'plots/Average_reward.png_{idx}')
        plt.close('all')
